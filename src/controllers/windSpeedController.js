...
const uploadWindSpeedData = async (req, res) => {
  try {
    const { data } = req.body;
    ...
    if (recordsToProcess.length > 0) {
      try {
        ...
        await WindSpeed.bulkCreate(recordDataArray, {
          updateOnDuplicate: [
        ...
        for (const record of recordsToProcess) {
          try {
            const existingRecord = await WindSpeed.findOne({
              where: {
            ...
            if (existingRecord) {
              ...
              await WindSpeed.create(record.recordData);
              results.successful++;
...
// Get all wind speed data with pagination and filters
const getAllWindSpeedData = async (req, res) => {
  try {
    const {
      page = 1,
      limit = 50,
    ...
    // Get total count
    const total = await WindSpeed.count({ where: whereClause });

    // Get paginated data
    const data = await WindSpeed.findAll({
      where: whereClause,
...
const getStations = async (req, res) => {
  try {
    const stations = await WindSpeed.findAll({
...
const getYears = async (req, res) => {
  try {
    const years = await WindSpeed.findAll({
...
const getWindSpeedDataById = async (req, res) => {
  try {
    const { id } = req.params;
    const data = await WindSpeed.findByPk(id);
...
const createWindSpeedData = async (req, res) => {
  try {
    const data = await WindSpeed.create(req.body);
...
const updateWindSpeedData = async (req, res) => {
  try {
    const { id } = req.params;
    const data = await WindSpeed.findByPk(id);
...
const deleteWindSpeedData = async (req, res) => {
  try {
    const { id } = req.params;
    const data = await WindSpeed.findByPk(id);
