...
const uploadSoilTemperatureData = async (req, res) => {
  try {
    const { data } = req.body;
    ...
    if (recordsToProcess.length > 0) {
      try {
        ...
        await SoilTemperature.bulkCreate(recordDataArray, {
          updateOnDuplicate: [
        ...
        for (const record of recordsToProcess) {
          try {
            const existingRecord = await SoilTemperature.findOne({ where: { station: record.recordData.station, year: record.recordData.year, month: record.recordData.month } });
            if (existingRecord) {
              ...
              await SoilTemperature.create(record.recordData);
              results.successful++;
...
const getAllSoilTemperatureData = async (req, res) => {
  try {
    ...
    const total = await SoilTemperature.count({ where: whereClause });
    const data = await SoilTemperature.findAll({ where: whereClause, limit: parseInt(limit), offset: offset, order: [[sortBy, sortOrder]] });
...
const getStations = async (req, res) => {
  try {
    const stations = await SoilTemperature.findAll({ attributes: [[sequelize.fn("DISTINCT", sequelize.col("station")), "station"]], order: [["station", "ASC"]], raw: true });
...
const getYears = async (req, res) => {
  try {
    const years = await SoilTemperature.findAll({ attributes: [[sequelize.fn("DISTINCT", sequelize.col("year")), "year"]], order: [["year", "DESC"]], raw: true });
...
const getSoilTemperatureDataById = async (req, res) => {
  try {
    const { id } = req.params;
    const data = await SoilTemperature.findByPk(id);
...
const createSoilTemperatureData = async (req, res) => {
  try {
    const data = await SoilTemperature.create(req.body);
...
const updateSoilTemperatureData = async (req, res) => {
  try {
    const { id } = req.params;
    const data = await SoilTemperature.findByPk(id);
    if (!data) return res.status(404).json({ success: false, message: "Record not found" });
...
const deleteSoilTemperatureData = async (req, res) => {
  try {
    const { id } = req.params;
    const data = await SoilTemperature.findByPk(id);
    if (!data) return res.status(404).json({ success: false, message: "Record not found" });
...
export { uploadSoilTemperatureData, getAllSoilTemperatureData, getStations, getYears, getSoilTemperatureDataById, createSoilTemperatureData, updateSoilTemperatureData, deleteSoilTemperatureData };
