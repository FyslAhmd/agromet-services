  ...
  try {
    ...
    if (recordsToProcess.length > 0) {
      try {
        ...
        await AverageTemperature.bulkCreate(recordDataArray, {
          updateOnDuplicate: [
        ...
        for (const record of recordsToProcess) {
          try {
            const existingRecord = await AverageTemperature.findOne({ where: { station: record.recordData.station, year: record.recordData.year, month: record.recordData.month } });
            if (existingRecord) {
              ...
              await AverageTemperature.create(record.recordData);
              results.successful++;
...
export { uploadAverageTemperatureData, getAllAverageTemperatureData, getStations, getYears, getAverageTemperatureDataById, createAverageTemperatureData, updateAverageTemperatureData, deleteAverageTemperatureData };
