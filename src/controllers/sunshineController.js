...
const uploadSunshineData = async (req, res) => {
  try {
    const { data } = req.body;
    ...
    if (recordsToProcess.length > 0) {
      try {
        ...
        await Sunshine.bulkCreate(recordDataArray, {
          updateOnDuplicate: [
        ...
        for (const record of recordsToProcess) {
          try {
            const existingRecord = await Sunshine.findOne({
              where: {
            ...
            if (existingRecord) {
              ...
              await Sunshine.create(record.recordData);
              results.successful++;
...
// Get all sunshine data with pagination and filters
const getAllSunshineData = async (req, res) => {
  try {
    const {
      page = 1,
      limit = 50,
    ...
    // Get total count
    const total = await Sunshine.count({ where: whereClause });

    // Get paginated data
    const data = await Sunshine.findAll({
      where: whereClause,
...
const getStations = async (req, res) => {
  try {
    const stations = await Sunshine.findAll({
...
const getYears = async (req, res) => {
  try {
    const years = await Sunshine.findAll({
...
const getSunshineDataById = async (req, res) => {
  try {
    const { id } = req.params;
    const data = await Sunshine.findByPk(id);
...
const createSunshineData = async (req, res) => {
  try {
    const data = await Sunshine.create(req.body);
...
const updateSunshineData = async (req, res) => {
  try {
    const { id } = req.params;
    const data = await Sunshine.findByPk(id);
...
const deleteSunshineData = async (req, res) => {
  try {
    const { id } = req.params;
    const data = await Sunshine.findByPk(id);
