...
const uploadSoilMoistureData = async (req, res) => {
  try {
    const { data } = req.body;
...
const getAllSoilMoistureData = async (req, res) => {
  try {
    ...
    const total = await SoilMoisture.count({ where: whereClause });
    const data = await SoilMoisture.findAll({ where: whereClause, limit: parseInt(limit), offset: offset, order: [[sortBy, sortOrder]] });
...
const getStations = async (req, res) => {
  try {
    const stations = await SoilMoisture.findAll({ attributes: [[sequelize.fn("DISTINCT", sequelize.col("station")), "station"]], order: [["station", "ASC"]], raw: true });
...
const getYears = async (req, res) => {
  try {
    const years = await SoilMoisture.findAll({ attributes: [[sequelize.fn("DISTINCT", sequelize.col("year")), "year"]], order: [["year", "DESC"]], raw: true });
...
const getSoilMoistureDataById = async (req, res) => {
  try {
    const { id } = req.params;
    const data = await SoilMoisture.findByPk(id);
...
const createSoilMoistureData = async (req, res) => {
  try {
    const data = await SoilMoisture.create(req.body);
...
const updateSoilMoistureData = async (req, res) => {
  try {
    const { id } = req.params;
    const data = await SoilMoisture.findByPk(id);
    if (!data) return res.status(404).json({ success: false, message: "Record not found" });
...
const deleteSoilMoistureData = async (req, res) => {
  try {
    const { id } = req.params;
    const data = await SoilMoisture.findByPk(id);
    if (!data) return res.status(404).json({ success: false, message: "Record not found" });
...
export { uploadSoilMoistureData, getAllSoilMoistureData, getStations, getYears, getSoilMoistureDataById, createSoilMoistureData, updateSoilMoistureData, deleteSoilMoistureData };
